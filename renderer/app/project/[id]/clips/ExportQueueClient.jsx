...
function ClipItemSkeleton() {
    return (<div className="surface flex flex-col gap-3 p-4 animate-pulse">
...
export default function ExportQueuePage() {
    ...
    useIpcEvent('clip:progress', (data) => {
    ...
    // Export progress
    useIpcEvent('export:progress', (data) => {
        setClips((prev) => prev.map((c) => c.id === data.clipId ? { ...c, status: 'processing' } : c));
    }, [projectId]);
    // Pipeline progress — reload clips when new ones are generated
    useIpcEvent('pipeline:progress', (data) => {
    ...
    const statusSummary = clips.length > 0
        ...
        : '';
    const actions = (<div className="flex items-center gap-3">
      {statusSummary && (<span className="text-xs font-mono text-text-secondary">{statusSummary}</span>)}
      {hooks.length > 0 && (<button type="button" onClick={() => void handleGenerateAll()} className={cn('flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5', 'text-xs font-medium text-text-secondary', 'hover:border-accent/40 hover:text-text-primary transition-micro', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent')}>
    ...
    return (<>
      <AppShell title="Export Queue" actions={actions}>
        <div className="mx-auto max-w-2xl px-6 py-6 flex flex-col gap-4">
          ...
          {/* Loading skeletons */}
          {loading && Array.from({ length: 4 }).map((_, i) => (<ClipItemSkeleton key={i}/>))}

          {/* Empty state */}
          {!loading && clips.length === 0 && (<div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
              ...
              <button type="button" onClick={() => electronNavigate(router, '/')} className={cn('mt-2 rounded-md border border-border px-4 py-2 text-sm text-text-secondary', 'hover:border-accent/40 hover:text-text-primary transition-micro', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent')}>
                ← Back to Dashboard
              </button>
            </div>)}
          ...
          {!loading && clips.length > 0 && (<>
              ...
              {clips.map((clip) => (<ClipItem key={clip.id} clip={clip} hook={hooks.find((h) => h.id === clip.hookId)} onRemoved={handleRemoved}/>))}
            </>)}
        </div>
      </AppShell>
