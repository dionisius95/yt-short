...
}).chain(({ startMs, summary, viralScore }) => endMsArb(startMs).map((endMs) => ({ startMs, endMs, summary, viralScore })));
/**
...
const rawHookWithArbitraryScoreArb = fc.record({
    startMs: startMsArb,
    summary: summaryArb,
}).chain(({ startMs, summary }) => endMsArb(startMs).chain((endMs) => 
// viralScore can be anything: out-of-range int, float, string, null, undefined
fc.oneof(fc.integer({ min: -10_000, max: 10_000 }), // integers (includes valid range)
fc.float({ min: -1e6, max: 1e6 }), // floats
fc.string(), // strings
fc.constant(null), fc.constant(undefined), fc.boolean()).map((viralScore) => ({ startMs, endMs, summary, viralScore }))));
/**
...
describe('Property 6 — parseHooks: viralScore is always in [0, 100]', () => {
    it('all returned hooks have viralScore as an integer in [0, 100] for any raw input array', () => {
        fc.assert(fc.property(fc.array(rawHookWithArbitraryScoreArb, { minLength: 0, maxLength: 30 }), projectIdArb, (rawArray, projectId) => {
    ...
    it('parseHooks returns empty array for non-array input', () => {
        fc.assert(fc.property(fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.constant(null), fc.constant(undefined), fc.record({ hooks: fc.array(validRawHookArb) })), projectIdArb, (nonArray, projectId) => {
    ...
    it('hooks with out-of-range viralScore are discarded entirely', () => {
        ...
        fc.oneof(fc.integer({ min: 101, max: 10_000 }), fc.integer({ min: -10_000, max: -1 })).map((viralScore) => ({ startMs, endMs, summary, viralScore })))), projectIdArb, (rawHook, projectId) => {
    ...
    it('hooks with valid viralScore in [0, 100] are always accepted', () => {
        fc.assert(fc.property(validRawHookArb, projectIdArb, (rawHook, projectId) => {
            const hooks = parseHooks([rawHook], projectId);
...
describe('Property 7 — clampHooks: result length is always in [0, 20]', () => {
    it('clampHooks always returns at most 20 hooks for any input size', () => {
        fc.assert(fc.property(fc.array(hookArb, { minLength: 0, maxLength: 30 }), projectIdArb, (hooks, projectId) => {
            const result = clampHooks(hooks, projectId);
            expect(result.length).toBeLessThanOrEqual(20);
        }), { numRuns: 200 });
    });
    it('when input has > 20 hooks, the top 20 by viralScore are kept', () => {
        fc.assert(fc.property(fc.array(hookArb, { minLength: 21, maxLength: 30 }), projectIdArb, (hooks, projectId) => {
            const result = clampHooks(hooks, projectId);
    ...
    it('when input has <= 20 hooks, the array is returned as-is', () => {
        fc.assert(fc.property(fc.array(hookArb, { minLength: 0, maxLength: 20 }), projectIdArb, (hooks, projectId) => {
            const result = clampHooks(hooks, projectId);
    ...
    it('when input has < 3 hooks, the array is returned as-is (caller handles error)', () => {
        fc.assert(fc.property(fc.array(hookArb, { minLength: 0, maxLength: 2 }), projectIdArb, (hooks, projectId) => {
            const result = clampHooks(hooks, projectId);
            expect(result).toHaveLength(hooks.length);
        }), { numRuns: 200 });
    });
