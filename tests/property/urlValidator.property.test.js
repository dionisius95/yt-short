 ...
 */
const videoIdArb = fc.stringOf(fc.mapToConstant({ num: 26, build: (n) => String.fromCharCode(65 + n) }, // A-Z
{ num: 26, build: (n) => String.fromCharCode(97 + n) }, // a-z
{ num: 10, build: (n) => String.fromCharCode(48 + n) }, // 0-9
{ num: 1, build: () => '-' }, { num: 1, build: () => '_' }), { minLength: 11, maxLength: 11 });
/**
 * Generates a valid standard watch URL.
 */
const watchUrlArb = videoIdArb.map((id) => `https://www.youtube.com/watch?v=${id}`);
/**
 * Generates a valid shortened youtu.be URL.
 */
const shortUrlArb = videoIdArb.map((id) => `https://youtu.be/${id}`);
/**
 * Generates a valid embed URL.
 */
const embedUrlArb = videoIdArb.map((id) => `https://www.youtube.com/embed/${id}`);
/**
 ...
 */
const nonYouTubeStringArb = fc.string({ minLength: 0, maxLength: 200 }).filter((s) => !s.includes('youtube.com') && !s.includes('youtu.be'));
/**
...
fc.stringOf(fc.mapToConstant({ num: 26, build: (n) => String.fromCharCode(65 + n) }, { num: 26, build: (n) => String.fromCharCode(97 + n) }, { num: 10, build: (n) => String.fromCharCode(48 + n) }), { minLength: 0, maxLength: 10 }), 
// Too long (12–30 chars)
fc.stringOf(fc.mapToConstant({ num: 26, build: (n) => String.fromCharCode(65 + n) }, { num: 26, build: (n) => String.fromCharCode(97 + n) }, { num: 10, build: (n) => String.fromCharCode(48 + n) }), { minLength: 12, maxLength: 30 }));
// ---------------------------------------------------------------------------
...
describe('Property 1a — valid YouTube URLs always pass validation', () => {
    it('watch URLs with valid 11-char video IDs are always accepted', () => {
        fc.assert(fc.property(watchUrlArb, (url) => {
    ...
    it('shortened youtu.be URLs with valid 11-char video IDs are always accepted', () => {
        fc.assert(fc.property(shortUrlArb, (url) => {
            const result = validateYouTubeUrl(url);
            expect(result.valid).toBe(true);
            if (result.valid) {
    ...
    it('embed URLs with valid 11-char video IDs are always accepted', () => {
        fc.assert(fc.property(embedUrlArb, (url) => {
            const result = validateYouTubeUrl(url);
            expect(result.valid).toBe(true);
            if (result.valid) {
...
describe('Property 1b — non-YouTube strings always fail validation with a reason', () => {
    it('arbitrary non-YouTube strings are always rejected with a non-empty reason', () => {
        fc.assert(fc.property(nonYouTubeStringArb, (input) => {
...
describe('Property 1c — watch URLs with invalid video IDs are always rejected', () => {
    it('watch URLs with non-11-char video IDs are always rejected', () => {
        fc.assert(fc.property(invalidVideoIdArb, (badId) => {
...
describe('Property 1d — extracted video ID always matches the input', () => {
    it('for any valid URL, the extracted videoId equals the ID used to construct the URL', () => {
        fc.assert(fc.property(videoIdArb, fc.oneof(fc.constant('watch'), fc.constant('short'), fc.constant('embed')), (id, format) => {
            let url;
            if (format === 'watch') {
...
describe('Property 1f — whitespace trimming is consistent', () => {
    it('a valid URL with surrounding whitespace is still valid', () => {
        fc.assert(fc.property(validYouTubeUrlArb, fc.string({ minLength: 0, maxLength: 5 }).filter((s) => s.trim() === ''), fc.string({ minLength: 0, maxLength: 5 }).filter((s) => s.trim() === ''), (url, leading, trailing) => {
