 ...
 */
const percentStrArb = fc.float({ min: -10, max: 110, noNaN: true }).map((n) => `${n.toFixed(1)}%`);
/**
 * Generates a non-empty speed string (e.g. "3.2 MiB/s").
 */
const speedArb = fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim().length > 0 && !s.includes('|'));
/**
 * Generates a non-empty eta string (e.g. "00:01:23").
 */
const etaArb = fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim().length > 0 && !s.includes('|'));
/**
 * Generates a well-formed progress line: "<percent>|<speed>|<eta>".
 */
const validProgressLineArb = fc.tuple(percentStrArb, speedArb, etaArb).map(([pct, speed, eta]) => `${pct}|${speed}|${eta}`);
// ---------------------------------------------------------------------------
...
describe('Property 2a — percent is always in [0, 100]', () => {
    it('parsed percent is always between 0 and 100 inclusive', () => {
        fc.assert(fc.property(validProgressLineArb, (line) => {
    ...
    it('values above 100 are clamped to 100', () => {
        fc.assert(fc.property(fc.float({ min: Math.fround(100.5), max: Math.fround(999), noNaN: true }), speedArb, etaArb, (overPercent, speed, eta) => {
    ...
    it('values below 0 are clamped to 0', () => {
        fc.assert(fc.property(fc.float({ min: Math.fround(-999), max: Math.fround(-0.5), noNaN: true }), speedArb, etaArb, (underPercent, speed, eta) => {
...
describe('Property 2e — invalid lines return null', () => {
    it('lines with no pipe separator return null', () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 100 }).filter((s) => !s.includes('|')), (line) => {
            const result = parseProgressLine(line);
            expect(result).toBeNull();
        }), { numRuns: 300 });
    });
    it('lines with only one pipe separator return null', () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 50 }).filter((s) => !s.includes('|')), fc.string({ minLength: 0, maxLength: 50 }).filter((s) => !s.includes('|')), (a, b) => {
            const line = `${a}|${b}`;
            const result = parseProgressLine(line);
            expect(result).toBeNull();
        }), { numRuns: 300 });
    ...
    it('lines with non-numeric percent field return null', () => {
        ...
        fc.string({ minLength: 1, maxLength: 20 }).filter((s) => isNaN(parseFloat(s.replace('%', '')))), speedArb, etaArb, (badPercent, speed, eta) => {
