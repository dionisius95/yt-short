...
describe('Property 8 — sortHooksByScore: output is sorted descending by viralScore', () => {
  it('every adjacent pair satisfies hooks[i].viralScore >= hooks[i+1].viralScore', () => {
    fc.assert(
      fc.property(
        fc.array(hookArb, { minLength: 0, maxLength: 50 }),
        (hooks) => {
          const sorted = sortHooksByScore(hooks);

          for (let i = 0; i < sorted.length - 1; i++) {
            expect(sorted[i].viralScore).toBeGreaterThanOrEqual(
              sorted[i + 1].viralScore
            );
          }
        }
      ),
  ...
  it('does not mutate the original array', () => {
    fc.assert(
      fc.property(
        fc.array(hookArb, { minLength: 1, maxLength: 50 }),
        (hooks) => {
    ...
    fc.assert(
      fc.property(
        ...
        (hooks) => {
          ...
          expect(sortedAgain.map((h) => h.viralScore)).toEqual(
            sorted.map((h) => h.viralScore)
          );
        }
      ),
      { numRuns: 200 }
