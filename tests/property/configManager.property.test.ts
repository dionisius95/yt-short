...
const whisperModelSizeArb = fc.constantFrom(
  'tiny', 'base', 'small', 'medium', 'large'
) as fc.Arbitrary<AppSettings['whisperModelSize']>;

const subtitleStyleArb = fc.constantFrom(
  'bold-white', 'gradient-pop', 'minimal-clean'
) as fc.Arbitrary<AppSettings['defaultSubtitleStyle']>;

const videoQualityArb = fc.constantFrom(
  '1080p', '720p', '480p', '360p'
) as fc.Arbitrary<AppSettings['defaultVideoQuality']>;

const subtitlePositionArb = fc.constantFrom(
  'lower-third', 'upper-third', 'center'
) as fc.Arbitrary<AppSettings['defaultSubtitlePosition']>;
...
describe('Property 25 — ConfigManager: set then get returns the same value', () => {
  ...
  it('whisperModelSize: set then get returns the same model size', () => {
    fc.assert(
      fc.property(whisperModelSizeArb, (value) => {
  ...
  it('defaultSubtitleStyle: set then get returns the same style', () => {
    fc.assert(
      fc.property(subtitleStyleArb, (value) => {
  ...
  it('defaultVideoQuality: set then get returns the same quality', () => {
    fc.assert(
      fc.property(videoQualityArb, (value) => {
  ...
  it('defaultSubtitlePosition: set then get returns the same position', () => {
    fc.assert(
      fc.property(subtitlePositionArb, (value) => {
  ...
  it('multiple independent set/get calls do not interfere with each other', () => {
    fc.assert(
      fc.property(
        ...
        nonEmptyStringArb,
        whisperModelSizeArb,
        (downloadDir, exportDir, whisperModelSize) => {
  ...
  it('getAll returns a snapshot containing all set values', () => {
    fc.assert(
      fc.property(
        ...
        videoQualityArb,
        subtitlePositionArb,
        (downloadDir, exportDir, whisperModelSize, subtitleStyle, videoQuality, subtitlePosition) => {
          const m = new ConfigManager();
