...
describe('Downloader — quality tier mapping', () => {
  ...
  const cases: Array<{ quality: string; heightToken: string }> = [
  ...
  for (const { quality, heightToken } of cases) {
    it(`maps '${quality}' to a format string containing '${heightToken}'`, async () => {
      mockSpawn.mockReturnValue(makeSuccessProc() as unknown as ReturnType<typeof spawn>);

      await downloader.download({
        ...BASE_REQ,
        quality: quality as '1080p' | '720p' | '480p' | '360p',
      });
...
describe('Downloader — retry logic', () => {
  ...
  it('retries up to 3 times total when yt-dlp always fails', async () => {
    ...
    (mockSpawn as any).mockImplementation(() => makeFailProc());
    ...
    downloadPromise.catch(() => { /* handled below */ });
...
describe('Downloader — cancel', () => {
  ...
  it('calls kill(SIGTERM) on the child process when cancel() is invoked', async () => {
    ...
    mockSpawn.mockReturnValue(hangingProc as unknown as ReturnType<typeof spawn>);
