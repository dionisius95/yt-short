...
// ---------------------------------------------------------------------------
const responseQueue = [];
const capturedReqBodies = [];
function queueHttpResponse(responseBody, statusCode = 200) {
...
function ollamaBody(text) {
    return JSON.stringify({ message: { role: 'assistant', content: text }, done: true });
}
// ---------------------------------------------------------------------------
// Test data
// ---------------------------------------------------------------------------
const VALID_HOOKS_JSON = '[{"startMs":1000,"endMs":30000,"viralScore":85,"summary":"Great moment"},' +
...
describe('Analyzer.detectHooks', () => {
        ...
        requestSpy.mockImplementation((opts, cb) => {
            ...
            if (!next) {
                ...
                return Object.assign(new EventEmitter(), {
                    write: vi.fn((chunk) => { capturedReqBodies.push(chunk); }),
                    end: vi.fn(),
                    setTimeout: vi.fn(),
                    destroy: vi.fn(),
                });
    ...
    describe('retry on JSON parse failure', () => {
        it('retries up to 2 times (3 total attempts) when Ollama returns invalid JSON', async () => {
            ...
            expect(thrown.code).toBe('INSUFFICIENT_HOOKS');
        ...
        it('throws AppError with code INSUFFICIENT_HOOKS after all retries are exhausted', async () => {
            ...
            expect(thrown.code).toBe('INSUFFICIENT_HOOKS');
            expect(thrown.message).toMatch(/after processing 1 chunk/);
        });
    ...
    describe('OllamaUnavailableError', () => {
        it('throws AppError with code OLLAMA_UNAVAILABLE when http request emits error', async () => {
            ...
            try {
                ...
                expect(thrown.code).toBe('OLLAMA_UNAVAILABLE');
            }
            finally {
        ...
        it('includes the original error message in the details field', async () => {
            ...
            try {
                ...
                expect(thrown.details).toMatch(/ECONNREFUSED/);
            }
            finally {
                global.setTimeout = originalSetTimeout;
            }
