...
describe('CHANNELS constants', () => {
  ...
  it('all channel values are non-empty strings', () => {
    for (const [key, value] of Object.entries(CHANNELS)) {
      ...
      expect((value as string).length, `CHANNELS.${key} should not be empty`).toBeGreaterThan(0);
    }
  });
  ...
  it('channel names follow the namespace:action pattern', () => {
    ...
    for (const [key, value] of Object.entries(CHANNELS)) {
      expect(
        colonPattern.test(value as string),
        `CHANNELS.${key} = "${value}" should match namespace:action pattern`
      ).toBe(true);
    }
  });
...
describe('PUSH_CHANNELS', () => {
  ...
  it('push channels are a subset of all CHANNELS values', () => {
    const allValues = new Set<string>(Object.values(CHANNELS));
    for (const ch of PUSH_CHANNELS) {
...
describe('registerIpcHandlers', () => {
  let ipcMain: { handle: ReturnType<typeof vi.fn>; removeHandler: ReturnType<typeof vi.fn> };
  ...
  it('registers a handler for every non-push channel', async () => {
    ...
    const internalOnlyChannels = new Set<string>([
      CHANNELS.UPLOAD_AUTH_CALLBACK,
    ]);

    // Every channel that is NOT a push channel and NOT internal-only should have a handler
    const pushSet = new Set<string>(PUSH_CHANNELS);
    const invokeChannels = Object.values(CHANNELS).filter(
      (ch) => !pushSet.has(ch as string) && !internalOnlyChannels.has(ch as string)
    );
    ...
    for (const channel of invokeChannels) {
      expect(
        ipcMain.handle,
        `ipcMain.handle should have been called with channel "${channel}"`
  ...
  it('does NOT register handlers for push (event) channels', async () => {
    ...
    for (const pushCh of PUSH_CHANNELS) {
      expect(
        registeredChannels,
        `Push channel "${pushCh}" should NOT have a handler registered`
      ).not.toContain(pushCh);
    }
  });
  ...
  it('project:list handler delegates to listProjects service', async () => {
    ...
    const call = ipcMain.handle.mock.calls.find(
      (c: unknown[]) => c[0] === CHANNELS.PROJECT_LIST
    );
  ...
  it('settings:get handler delegates to getSettings service', async () => {
    ...
    const call = ipcMain.handle.mock.calls.find(
      (c: unknown[]) => c[0] === CHANNELS.SETTINGS_GET
    );
    expect(call).toBeDefined();
  ...
  it('download:start handler passes request to startDownload service', async () => {
    ...
    const call = ipcMain.handle.mock.calls.find(
      (c: unknown[]) => c[0] === CHANNELS.DOWNLOAD_START
    );
    expect(call).toBeDefined();

    const handler = call![1] as (...args: unknown[]) => Promise<unknown>;
    const req = { url: 'https://youtube.com/watch?v=abc', quality: '1080p', outputDir: '/tmp' };
    const result = await handler({} as Electron.IpcMainInvokeEvent, req);
...
type PartialServices = Partial<import('../../electron/ipc/handlers').IpcServices>;

function buildStubServices(overrides: PartialServices = {}): import('../../electron/ipc/handlers').IpcServices {
  return {
